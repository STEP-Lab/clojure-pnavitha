...
(defn conditions-apply
  ...
  {:level        :medium
   ...
   :implemented? true}
  [coll]
  (condp #(= %1 (filter (set %1) %2)) (filter #{1 3 :a :b :c [2 3] [4 5]} coll)
    [1 3] :wonder-woman
    [:a :b :c] :durga
    [[2 3] [4 5]] :cleopatra
    :tuntun))
...
(defn zero-separated-palindrome
  ...
  {:level :easy
   ...
   :implemented? true}
  [coll]
  (as-> (map inc coll) incremented-list
        (concat (reverse incremented-list)[0] incremented-list))
  )